...
_TEMPLATE_HEADER = (
    "# 历史决赛作品清单（src.ingest 数据获取输入）\n"
    "# 每条记录字段：repo_url / year / team_name / award_level\n"
    "# 仓库会被克隆到 data/repos/{year}/{team_name}/\n"
    "# 下面 3 条为示例，请替换为真实仓库后运行：python -m src.ingest --config config/repos.yaml\n"
)
...
def load_repos(path: str | Path) -> list[RepoEntry]:
    ...
    if not path.exists():
        raise FileNotFoundError(
            f"配置文件不存在：{path}。可先运行 `python -m src.ingest --init-template` 生成模板。"
        )
