"""embed 模块 CLI：
  ...
  python -m oskernel_agent.comparison.embed build --all
  python -m oskernel_agent.comparison.embed query --repo <新作品路径>
...
from oskernel_agent.comparison.normalize.store import DEFAULT_DB as FUNCTIONS_DB
...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(prog="python -m oskernel_agent.comparison.embed", description="函数级代码嵌入与向量检索。")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.cmd == "query":
        ...
        if args.with_simhash:
            from oskernel_agent.comparison.simhash.build import DEFAULT_IDF, DEFAULT_INDEX, SimHashQuery
