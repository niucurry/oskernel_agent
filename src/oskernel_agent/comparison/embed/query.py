...
from oskernel_agent.comparison.exact.identity import (
    MIN_IDENTITY_SCORE,
    FunctionIdentityFeatures,
    compare_function_identity_features,
    function_identity_features,
)
from oskernel_agent.comparison.normalize.runner import derive_repo_id, normalize_repo
from oskernel_agent.comparison.normalize.store import FunctionStore as NormStore, normalized_code_hash
...
def query_repo(
    ...
    *,
    ...
    simhash_query 给定（oskernel_agent.comparison.simhash.build.SimHashQuery）时，SimHash 作为**并集补充通道**：
    ...
    if _fi is not None:
        ...
        logger.warning("[{}] faiss 索引不存在，回退 qdrant-local（慢）；可运行 "
                       "`python -m oskernel_agent.comparison.embed build-faiss` 构建", repo_id)
        ...
        results.append(
            {
                "query": {
                    ...
                    "module_tag": row["module_tag"],
                    "lang": row["lang"],
                    # 供 Layer 4（oskernel_agent.comparison.exact）精确比对使用
