    ...
    def search(
        ...
        *,
        ...
        if candidate_ids is not None:
            ...
            distances, indices = flat.search(q, k)
            found = [
                (row_func_ids[int(index)], float(distances[0][pos]))
                for pos, index in enumerate(indices[0]) if index >= 0
            ]
        else:
            ...
            found = []
            while search_k > 0:
                distances, indices = self._idx.search(q, search_k)
                pairs = [
                    (int(self._ids[index]), float(distances[0][pos]))
                    for pos, index in enumerate(indices[0]) if index >= 0
                ]
