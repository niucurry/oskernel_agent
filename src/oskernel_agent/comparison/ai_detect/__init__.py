...
- 复用本仓库 `oskernel_agent.comparison.normalize` 的 tree-sitter 解析（rust/c）抽取函数，桥接为上游 FunctionBlock，
  避免引入与本仓库冲突的 `tree-sitter-languages`（见 vendor/README.md）。
- 检测结果落盘 `{repo}_ai_detect.json`，与其他模块一样文件级解耦。
- 报告环节由 `oskernel_agent.comparison.report` 单独「唤起一个会话」生成「AI 生成代码检测」章节并拼入最终报告。

CLI：`python -m oskernel_agent.comparison.ai_detect --repo <路径>`
"""
