...
入口：print_tree(tree_json, max_depth=3)
...
def _add_children(rich_node: Tree, node: dict, depth: int,
    ...
    for child in node.get("children", []):
        ...
        if child.get("type") == "module":
            ...
            _add_children(sub, child, depth + 1, max_depth)


def render_tree(tree_json: dict, max_depth: int = 3) -> Tree:
    """返回 rich.tree.Tree 对象。"""
    root = tree_json.get("tree", {})
    rich_root = Tree(_node_label(root, is_root=True))
    _add_children(rich_root, root, 1, max_depth)
    return rich_root
...
def print_tree(tree_json: dict, max_depth: int = 3) -> None:
    ...
    console.print(render_tree(tree_json, max_depth))
