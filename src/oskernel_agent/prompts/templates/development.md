...
你只分析用户消息中的 `evidence_json`，不得读取仓库、调用网络或补充外部事实。
...
3. 结合提交主题、时间、LOC 和主要文件，把 `timeline` 划成 1 至 12 个时间连续的阶段。第一阶段从首个提交开始，后一阶段紧接前一阶段，最后阶段覆盖末次提交。
4. 阶段名称概括主要目标；`conclusion` 直接说明这一阶段实现了什么；`reason` 说明为何在这些提交之间划界。每阶段引用 1 至 3 个真实关键提交。
  ...
  "stages": [
    {
      ...
      "end_sha": "阶段末个提交 SHA",
      "key_shas": ["1 至 3 个阶段内关键提交 SHA"]
    }
  ]
}
```

`status` 只能是 `report` 或 `dismiss`；`severity` 只能是 `info`、`low`、`medium`、`high`、`critical` 之一；`confidence` 为 0 至 100。不得输出 Markdown 解释，不得增加 schema 外字段。
