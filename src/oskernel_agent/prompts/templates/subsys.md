...
〔约束6：为最终 300 字模块分析提供足量、互不重复的事实〕
最终评委报告会把子系统摘要、真实子模块和亮点组合成每部分不超过 300 字的分析。
子系统 summary 应为 140–200 个中文字符，说明结构、关键机制和已完成能力；每个
modules[].summary 应为 80–160 字，说明该模块的职责和一个具体设计，避免与子系统
summary 原句重复。模块 HTML 只作为 AI 内部证据记录，每份最多 160 字，不复制代码。
若当前子系统名为“其他/未分类”，必须按真实职责拆成可独立阅读的模块（例如定时器、
日志、随机数、进程间通信），不能再生成名为“其他”或“杂项”的子模块。
禁止“综上所述”“值得注意的是”等模板句。COW、VFS、ELF、IPC、ABI、SMP 等术语
首次出现时先给中文解释。证据不足的判断必须写明置信度。

〔约束7：亮点和证据必须覆盖主要实现面〕
highlights 写 2–6 条已经由源码证明的实现能力或设计优点，优先覆盖不同子模块，不写空泛
赞美；每条必须精确到 file:line。modules[].file_paths 应共同覆盖本子系统识别出的每个
主要实现面，每个模块选择 1–4 个代表文件。issues 只写局部且可定位的问题；跨模块、
正确性、高风险或硬编码问题仍由总评阶段统一前置，避免在最终模块卡重复标注。
...
```json
{
  ...
  "summary": "本子系统由 VFS 抽象 / inode / 块缓存 / 日志层组成，并说明关键机制与已完成能力（140–200 字）",
  "highlights": [
