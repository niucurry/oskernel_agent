...
_COMPILE_TOPIC_RE = re.compile(
    # 门禁针对构建状态断言（编译通过/失败、构建入口、双架构编译等），
    # 必须带状态词才算命中；「重编译测例」「构建类评测」等事实性表述不再误伤。
    r"(?:(?:双架构|镜像|交叉|正式|重新|无法|可以|可否|能否|通过|未通过|失败|成功|完整)(?:编译|构建))"
    r"|(?:(?:编译|构建)(?:通过|失败|成功|未过|不通过|验证|测试|检查|结果|完成|未完成|入口|环境|依赖|报错|错误|选项|参数))"
    r"|\b(?:make)\b"
